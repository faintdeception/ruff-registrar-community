...
A comprehensive student management system built with .NET Aspire, Keycloak, PostgreSQL, and Next.js!
...
- **Database**: PostgreSQL 17 locally through Aspire
...
- Publish/deploy mode requires secrets from configuration or the hosting environment.
- Development overrides should be stored in user secrets or environment variables when needed.

### Accessing Service Credentials
- Navigate to http://localhost:15888 after starting the application.
- View the Resources tab to see generated connection strings and service endpoints.
- Local development defaults are intentionally simple; production deployments must provide real secrets.
...
1. **User Secrets**: Sensitive local overrides can be stored in .NET user secrets.
2. **Environment Variables**: Alternative configuration is available through environment variables.
3. **Local Defaults**: The AppHost includes development-only defaults for quick local startup; deployment mode requires explicit secret values.

### Setting Up Secrets

**Note:** When using .NET Aspire locally, the AppHost uses fixed development defaults with persisted local volumes. These manual configuration options are needed for Docker Compose deployments, custom setups, and production-like environments.
