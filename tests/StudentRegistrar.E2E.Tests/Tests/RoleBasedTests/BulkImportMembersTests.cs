...
public class BulkImportMembersTests : BaseRoleNavigationTest
{
    ...
    public void Admin_Should_Bulk_Import_Members_From_Csv()
    {
        ...
        var csvPath = Path.Combine(Path.GetTempPath(), $"bulk-import-{timestamp}.csv");
        File.WriteAllText(csvPath, $"firstName,lastName,email\nBulk,Import{timestamp},{email}\n");
    ...
    public void Admin_Should_See_Row_Level_Errors_For_Invalid_Csv_Rows()
    {
        ...
        var csvPath = Path.Combine(Path.GetTempPath(), $"bulk-import-mixed-{timestamp}.csv");
        // Second row has a blank email, which must fail without aborting the whole batch.
        File.WriteAllText(csvPath,
            "firstName,lastName,email\n" +
            $"Good,Row{timestamp},{goodEmail}\n" +
            $"Bad,Row{timestamp},\n");
